...
const callOpenRouterAPI = async (text, model, apiKey, options = {}) => {
  ...
  const systemPrompt = options.type === 'page'
    ? 'You are an expert study assistant. Summarize the following text using  the 80/20 principle: - Extract only the key ideas that give most of the understanding. -Ignore filler, repetition, and minor examples. -Focus on what a student should remember. -Use clear bullet points. -Keep it concise but meaningful. -If the text is unclear or fragmented, still infer the most likely core ideas carefully. Return: 1. 2-3 essential bullet points. 2. 1-2 sentence "core takeaway". 3. Likely Exam Points (3 bullets). 4. likely exam questions (2 bullets).'
    : 'You are an AI study assistant helping students learn faster. Summarize the following pages using the 80/20 rule: extract the small number of ideas that give the student the biggest understanding of the pages. Rules:- Include only the most important concepts, arguments, definitions, and facts. - Prioritize what is most useful for understanding and exam revision. - Skip minor examples, repetition, storytelling, and low-value details. - Use simple, direct language. - Format the output as clean bullet points. - Group points into short logical sections if needed. - Emphasize any key definitions, processes, comparisons, formulas, or cause/effect relationships. - Do NOT rewrite the whole pages. - Keep it concise, but make sure the student can grasp the pages’ core ideas from the summary alone. Output format: 1. Multi-Page Summary (bullet points) 2. Core Takeaway (1–2 sentences) 3. Likely Exam Points (3–5 bullets). 4. likely exam questions (3 bullets). Chapter: {{CHAPTER_TEXT}}';
